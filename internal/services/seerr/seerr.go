...
// ErrSeerr is a custom error type for Seerr-specific errors
type ErrSeerr struct {
	Message string
	Errors  []string
}

func (e *ErrSeerr) Error() string {
	if len(e.Errors) == 0 {
		return e.Message
	}
...
const (
	seerrTitleLookupTimeout  = 3 * time.Second
	seerrTitleCacheTTL       = 30 * time.Minute
	seerrTitleLookupParallel = 4
)
...
func init() {
	models.NewSeerrService = NewSeerrService
}

func NewSeerrService() models.ServiceHealthChecker {
	service := &SeerrService{}
	service.Type = "seerr"
	service.DisplayName = "Seerr"
	service.Description = "Monitor and manage your Seerr instance"
...
func (s *SeerrService) GetHealthEndpoint(baseURL string) string {
	baseURL = strings.TrimRight(baseURL, "/")
	return fmt.Sprintf("%s/api/v1/status", baseURL)
}

// SetDB sets the database instance for the service
func (s *SeerrService) SetDB(db *database.DB) {
	s.db = db
}

// UpdateRequestStatus updates the status of a media request (approve/reject)
func (s *SeerrService) UpdateRequestStatus(ctx context.Context, url, apiKey string, requestID int, approve bool) error {
	if url == "" {
		return &ErrSeerr{Message: "Configuration error", Errors: []string{"URL is required"}}
	}
	if apiKey == "" {
		return &ErrSeerr{Message: "Configuration error", Errors: []string{"API key is required"}}
	}
	...
	if err != nil {
		return &ErrSeerr{Message: "Connection error", Errors: []string{err.Error()}}
	}
...
func (s *SeerrService) GetRequests(ctx context.Context, url, apiKey string) (*types.RequestsStats, error) {
	if url == "" {
		return nil, &ErrSeerr{Message: "Configuration error", Errors: []string{"URL is required"}}
	}
	...
	if err != nil {
		return nil, &ErrSeerr{Message: "Connection error", Errors: []string{err.Error()}}
	}

	body, err := s.ReadBody(resp)
	if err != nil {
		return nil, &ErrSeerr{Message: "Service error", Errors: []string{err.Error()}}
	}

	var requestsResponse types.RequestsResponse
	if err := json.Unmarshal(body, &requestsResponse); err != nil {
		return nil, &ErrSeerr{Message: "Response error", Errors: []string{"Failed to parse requests response"}}
	}
...
func setCachedTitle(cacheKey, title string) {
	...
	titleCache[cacheKey] = titleCacheEntry{
		title:     title,
		expiresAt: time.Now().Add(seerrTitleCacheTTL),
	}
	titleCacheMu.Unlock()
}
...
func (s *SeerrService) fetchTitle(ctx context.Context, baseURL, apiKey, mediaType string, tmdbID int) (string, error) {
	if tmdbID <= 0 {
		return "", nil
	}
...
func (s *SeerrService) enrichMissingRequestTitles(ctx context.Context, baseURL, apiKey string, requests []types.MediaRequest) {
	...
	group.SetLimit(seerrTitleLookupParallel)

	for _, lookup := range lookups {
		group.Go(func() error {
			lookupCtx, cancel := context.WithTimeout(groupCtx, seerrTitleLookupTimeout)
			defer cancel()
...
func (s *SeerrService) CheckHealth(ctx context.Context, url, apiKey string) (models.ServiceHealth, int) {
	startTime := time.Now()

	if url == "" {
		return s.CreateHealthResponse(startTime, "error", (&ErrSeerr{
	...
	if err != nil {
		return s.CreateHealthResponse(startTime, "offline", (&ErrSeerr{
	...
	if err := json.Unmarshal(body, &statusResponse); err != nil {
		return s.CreateHealthResponse(startTime, "warning", (&ErrSeerr{
	...
	if err := s.CacheVersion(ctx, url, statusResponse.Version, time.Hour); err != nil {
		log.Warn().
			...
			Msg("Failed to cache Seerr version")
	}
