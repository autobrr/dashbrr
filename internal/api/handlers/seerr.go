...
const (
	seerrStaleDataDuration = 5 * time.Minute
	seerrCachePrefix       = "seerr:requests:"
)
...
func NewSeerrHandler(db *database.DB, cache cache.Store, bc *Broadcaster) *SeerrHandler {
	return &SeerrHandler{
		db:               db,
		cache:            cache,
		bc:               bc,
...
func (h *SeerrHandler) UpdateRequestStatus(c *gin.Context) {
	...
	_, err, _ = h.sf.Do(sfKey, func() (any, error) {
		return nil, resilience.RetryWithBackoff(ctx, func() error {
			return service.UpdateRequestStatus(ctx, seerrConfig.URL, seerrConfig.APIKey, reqID, approve)
		})
	})
	...
	cacheKey := seerrCachePrefix + instanceId
	...
	if err == nil && result != nil {
		...
		h.broadcastSeerrRequests(instanceId, stats)
	}

	c.Status(http.StatusOK)
}

func (h *SeerrHandler) GetRequests(c *gin.Context) {
	instanceId, ok := requireInstanceID(c, "seerr", "Seerr")
	if !ok {
		return
	}
	...
	statsVal, err := FetchWithSWRCache(ctx, SWRCacheOptions[types.RequestsStats]{
		Store:           h.cache,
		Key:             cacheKey,
		FreshTTL:        middleware.CacheDurations.SeerrRequests,
		StaleTTL:        seerrStaleDataDuration,
	...
	if err != nil {
		...
		if err == context.DeadlineExceeded || err == context.Canceled {
			...
			log.Error().Err(err).Str("instanceId", instanceId).Msg("Request timeout while fetching Seerr requests")
		} else {
			log.Error().Err(err).Str("instanceId", instanceId).Msg("Failed to fetch Seerr requests")
		}
	...
	h.hashMu.Lock()
	currentHash, changes := createSeerrRequestsHash(stats)
	...
	if len(stats.Requests) > 0 && (lastHash == "" || currentHash != lastHash) {
		log.Debug().
			...
			Msg("[Seerr] Successfully retrieved and cached requests")
		...
		if lastHash != "" && currentHash != lastHash {
			log.Debug().
				...
				Msg("Seerr requests hash changed")
		}
...
func (h *SeerrHandler) fetchRequests(ctx context.Context, instanceId string) (*types.RequestsStats, error) {
	seerrConfig, err := requireServiceConfigLegacy(ctx, h.db, instanceId)
	if err != nil {
		return nil, err
	}
	...
	stats, err := service.GetRequests(ctx, seerrConfig.URL, seerrConfig.APIKey)
	if err != nil {
		return nil, err
	}
...
func (h *SeerrHandler) broadcastSeerrRequests(instanceId string, stats *types.RequestsStats) {
	if stats == nil {
		return
	}

	publishInternalServiceUpdate(h.bc, buildSeerrRequestsServiceUpdate(instanceId, stats))
}

// createSeerrRequestsHash generates a deterministic hash of the requests state
func createSeerrRequestsHash(stats *types.RequestsStats) (string, []string) {
