...
type mockServiceCreator struct {
	createServiceFunc func(serviceType string) services.ServiceHealthChecker
}

func (m *mockServiceCreator) CreateService(serviceType string) services.ServiceHealthChecker {
...
func TestHealthHandler_CheckHealth(t *testing.T) {
	...
	tests := []struct {
		name           string
		serviceID      string
		mockDBResponse func(ctx context.Context, params domain.FindServiceParams) (*domain.ServiceConfiguration, error)
		mockHealth     func(ctx context.Context, url, apiKey string) (domain.ServiceHealth, int)
		expectedCode   int
		expectedBody   gin.H
	}{
		{
