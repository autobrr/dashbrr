...
} from "../src/components/services/seerr/status.ts";
import type { SeerrMediaRequest } from "../src/types/service.ts";

const baseRequest = (
  overrides: Partial<SeerrMediaRequest>
): SeerrMediaRequest =>
  ({
    id: 1,
    status: 0,
...
test("keeps pending request actions when status is string encoded", () => {
  ...
  assert.equal(pending.requestStatus, SEERR_REQUEST_STATUS.PENDING);
  assert.equal(pending.meta.label, "Pending");
});

test("matches seerr request status enum values", () => {
  assert.deepEqual(SEERR_REQUEST_STATUS, {
    PENDING: 1,
    APPROVED: 2,
    DECLINED: 3,
    FAILED: 4,
...
test("matches seerr media status enum values", () => {
  assert.deepEqual(SEERR_MEDIA_STATUS, {
    UNKNOWN: 1,
    PENDING: 2,
    PROCESSING: 3,
...
test("prefers media lifecycle for approved requests when media state exists", () => {
  const resolved = resolveRequestStatus(
    baseRequest({
      status: SEERR_REQUEST_STATUS.APPROVED,
      media: { ...baseRequest({}).media, status: SEERR_MEDIA_STATUS.AVAILABLE },
    })
  );
