...
import { SeerrMediaRequest } from "../../../types/service";
...
interface SeerrStatsProps {
  instanceId: string;
}

const EMPTY_REQUESTS: SeerrMediaRequest[] = [];

export const SeerrStats: React.FC<SeerrStatsProps> = ({
  instanceId,
}) => {
  const { t } = useTranslation();
  const { getService } = useServiceData();
  const service = getService(instanceId);
  const serviceRequests = service?.stats?.seerr?.requests ?? EMPTY_REQUESTS;
  ...
  const pendingRequests = useMemo(
    () =>
      requests.filter(
        (req) =>
          getRequestStatus(req.status) === SEERR_REQUEST_STATUS.PENDING
      ),
    [requests]
  );
  ...
  const [selectedRequest, setSelectedRequest] =
    useState<SeerrMediaRequest | null>(null);
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      await api.post(
        `/api/services/${instanceId}/seerr/request/${selectedRequest.id}/${status}`
      );
  ...
  const getUserDisplayName = (
    requestedBy: SeerrMediaRequest["requestedBy"]
  ) => {
  ...
  const getMediaType = (request: SeerrMediaRequest) => {
    return request.media.tvdbId ? t("seerr.show", "Show") : t("seerr.movie", "Movie");
  };

  const getMediaTitle = (request: SeerrMediaRequest) => {
  ...
  const RequestItem = ({
    ...
    const isPendingRequest =
      status.requestStatus === SEERR_REQUEST_STATUS.PENDING;
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {isPendingRequest ? (
                ...
                >
                  {t(`seerr.status.${statusMeta.label.toLowerCase().replace(" ", "_")}`, statusMeta.label)}
